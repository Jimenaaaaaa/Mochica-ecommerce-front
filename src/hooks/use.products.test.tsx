...
describe("Given the useProducts Custom Hook, an ApiRepo and a given component", () => {
  ...
  //   test("Then, the ProductRegister function should be called", async () => {
  //     const elements = await screen.findAllByRole("button");
  //     await act(async () => ProductEvent.click(elements[0]));
  //     expect(mockRepo.registerProduct).toHaveBeenCalled();
  //   });

  describe("When the TestComponent is rendered and the GetAll button is clicked", () => {
    test("Then, the productsGetAll function should be called", async () => {
      const elements = await screen.findAllByRole("button");
  ...
  describe("When the TestComponent is rendered and the getByTag button is clicked", () => {
    test("Then, the productsgetByTag function should be called", async () => {
      const elements = await screen.findAllByRole("button");
